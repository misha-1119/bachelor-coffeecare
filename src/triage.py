...
GREET_RE = re.compile(
    r"^(привіт|приветствую|здоров|добрий\s+(день|ранок|вечір)|добридень|добрий|"
    r"доброго\s+(ранку|дня|вечора)|доброї\s+ночі|хай|hi|hello|hey)\b",
    re.IGNORECASE,
)

GOODBYE_RE = re.compile(
    r"^(дякую|спасибі|до\s+побачення|па|бувайте|бувай|поки|всього\s+доброго|"
    r"досвидання|чао|все\s+дякую|дякую\s+все|thanks|thank\s+you|bye|goodbye|ciao)\b",
    re.IGNORECASE,
)
...
URGENT_TRIGGERS_RE = re.compile(
    ...
    r"горілий\s+пластик|горілим\s+пластик|горілою\s+гум|запах\s+горілого|"
    r"пахне\s+горіл|smoke|burning\s+smell|burning\s+plastic)",
    re.IGNORECASE,
)
